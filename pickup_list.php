...
//
// $Id: pickup_list.php 50 2008-07-09 15:54:07Z frey $
//
...
<table class="UD_form" cellpadding="4">
  <tr class="UD_form_header"><td>Claim ID</td><td>Sender</td><td>Created</td></tr>
<?PHP
      $i = 0;
      while ( $i < $iMax ) {
        //
        ...
        ?><td><tt><?PHP
        $dropoffDate = timeForDate($allDropoffs[$i]->created());
        echo strftime("%d %b %Y&nbsp;&nbsp;%r",$dropoffDate);
        ?></tt></td></tr><?PHP
        $i++;
      }
?>
  <tr class="UD_form_footer"><td colspan="3" align="center"><?=$tableLabel?></td></tr>
</table>
<br/>
...
<?PHP
      ...
      ?><h5>There are no drop-offs available for you at this time.</h5><?PHP
    }
  } else {
    $theDropbox->HTMLWriteHeader();
    $theDropbox->HTMLStartBody();
